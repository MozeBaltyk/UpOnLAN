...
> TFTP on port 69 is the classic conflict: any other TFTP server on the same host/interface (e.g. a leftover KVM `dnsmasq` config) will block it. See [DHCP insights](Networks/02-DHCP.md).

---

## 🛑 PXE boot fails

Walk the boot steps from [Networks summary](Networks/01-Overview.md):

| Step | Failure | Check |
|------|---------|-------|
| 1️⃣ DHCP assigns IP + iPXE path | No IP / no boot file offered | DHCP config (`next-server`/`filename`) — [DHCP insights](Networks/02-DHCP.md) |
