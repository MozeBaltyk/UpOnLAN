...
## Task
Make Getting Started the first webapp documentation item; expand it with first workflows and expected results; position PXE Basics immediately after it as the component/definition reference.

## Planner
- Scope: rename only the two targeted files if needed for the current alphabetical tree navigation; improve their content and preserve links.
- Acceptance criteria: Getting Started shows an actionable first-run sequence with observable outcomes; Basics contains an ASCII diagram, component list, and definitions.

## Debater
- Avoid recommending boilerplate pages or duplicating generated/reference material.
- The current navigation is alphabetical; use simple numeric prefixes for these two user-facing entry pages rather than modifying application navigation logic.
...
- Implementation must hide only a leading numeric ordering prefix in the rendered navigation while retaining the real filename/path.

## Tester
- Pending final validation of ordered documentation pages and navigation label test.
...
## Commit
- Pending: stage the intended documentation, doc navigation source/test, and workflow-state changes; do not stage `.codegraph/`.
